...
const Main = () => {
    ...
    const { logData, signguNm, setSignguNm } = useContext(ProjectContext);
    const token = window.localStorage.getItem("token");
    const params =  {areaCd: areaCd,};  // 쿼리 파라미터로 전달
   
useEffect(() => {
    if (areaCd) {
        requestSignguNm();
    }
}, [areaCd]); // areaCd가 변경될 때마다 실행

const requestSignguNm = async () => {
    const response = await axios.get(`${API_BASE_URL}/1`,{
        params: { areaCd: areaCd }, // 쿼리 파라미터로 전달
        headers: {
          Authorization: `Bearer ${logData}`, // 필요 시 인증 헤더 추가
        },
    })
    ...
    const contentEvent = () => {
        switch (select) {
            ...
            case "경기":
                ...
                break;
            case "인천":
                setContent({ english: "INCHEON", explan: "인천은 아름다운 바다와 다양한 문화가 어우러진 도시로, 차이나타운과 송도 국제도시가 유명합니다. 또한, 인천공항을 중심으로 편리한 교통과 풍성한 쇼핑, 맛집도 즐길 수 있는 여행지입니다." });
                break;
            case "강원":
                setContent({ english: "GANGWON", explan: "강원도는 청정 자연과 아름다운 산과 바다가 어우러진 곳으로, 설악산과 동해의 해변이 유명합니다. 하이킹, 스키, 해양 스포츠 등 다양한 활동을 즐기며 자연의 매력을 만끽할 수 있는 여행지입니다." });
                break;
            case "충청북도":
                setContent({ english: "CHUNCHEONG", explan: "충청도는 고즈넉한 전통과 자연의 아름다움이 어우러진 지역으로, 공주와 부여의 역사 유적지와 청풍호수의 경치가 유명합니다. 또한, 맛있는 음식과 다양한 축제들이 있어 여행객들에게 풍성한 경험을 제공합니다." });
                break;
            case "충청남도":
                setContent({ english: "CHUNCHEONG", explan: "충청도는 고즈넉한 전통과 자연의 아름다움이 어우러진 지역으로, 공주와 부여의 역사 유적지와 청풍호수의 경치가 유명합니다. 또한, 맛있는 음식과 다양한 축제들이 있어 여행객들에게 풍성한 경험을 제공합니다." });
                ...
                break;
            case "전라북도":
                setContent({ english: "JEOLLA", explan: "전라도는 풍부한 역사와 문화유산을 자랑하는 지역으로, 전주 한옥마을과 광주 문화가 유명합니다. 또한, 맛있는 음식과 아름다운 자연 경관, 특히 남해안의 섬들이 매력적인 여행지입니다." });
                break;
            case "전라남도":
                setContent({ english: "JEOLLA", explan: "전라도는 풍부한 역사와 문화유산을 자랑하는 지역으로, 전주 한옥마을과 광주 문화가 유명합니다. 또한, 맛있는 음식과 아름다운 자연 경관, 특히 남해안의 섬들이 매력적인 여행지입니다." });
                break;
            case "경상북도":
                setContent({ english: "GYEONGSANG", explan: "경상도는 풍부한 역사와 자연을 자랑하는 지역으로, 부산의 해변과 경주의 고대 유적이 매력적입니다. 전통적인 문화와 맛있는 음식도 함께 즐길 수 있어 여행하기 좋은 곳입니다." });
                break;
            case "경상남도":
                setContent({ english: "GYEONGSANG", explan: "경상도는 풍부한 역사와 자연을 자랑하는 지역으로, 부산의 해변과 경주의 고대 유적이 매력적입니다. 전통적인 문화와 맛있는 음식도 함께 즐길 수 있어 여행하기 좋은 곳입니다." });
                break;
            case "제주":
                setContent({ english: "JEJU", explan: "제주도는 아름다운 자연경관과 독특한 문화가 어우러진 섬으로, 한라산과 해변, 오름을 탐험할 수 있습니다. 신선한 해산물과 전통적인 음식도 즐기며 휴식을 취하기 좋은 여행지입니다." });
                break;
            default:
